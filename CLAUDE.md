...
pnpm docs:api              # Generate docs/api from public JSDoc
pnpm docs:api:check        # Fail if generated API docs are stale
...
1. `README.md` for package-level installation, requirements, examples, and
   operational caveats.
2. Generated TypeDoc HTML under `docs/api/`, produced by `pnpm docs:api`.
3. TypeScript declarations generated by `pnpm build` under `dist/**/*.d.ts`.
4. Inline JSDoc in `src/shared/types.ts`, `src/shared/factory.ts`,
...
run `pnpm build` and `pnpm docs:api`. CI runs `pnpm docs:api:check`, which
regenerates `docs/api/` and fails if the committed reference is stale.
